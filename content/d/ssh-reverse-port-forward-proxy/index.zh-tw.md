...
draft: false
title: "ssh -R 一劍開天門：讓中國大陸的伺服器借用本機代理下載海外相依套件"
...
本文處理的是一個特定情境：在中國大陸部署伺服器時，網路審查可能讓海外開發資源的連線變得不穩定。這時一到複製儲存庫或部署服務，就可能突然出狀況：網路逾時、TLS 交握失敗，或某個相依套件下載到一半卡住。

你可能辛苦地把 npm、PyPI、Go、apt/dnf 等工具改用中國大陸的鏡像站，卻仍然無法解決所有問題，因為：
...
結束目前的 `ssh -R` 連線，一切就會恢復原狀。這個命令本身不可能讓伺服器失去連線，也不會影響同時登入伺服器的其他使用者。


> 在中國大陸，使用這類網路代理軟體不一定違反法規。企業可以合法申請網際網路專線；中國大陸各地電信業者提供的 SD-WAN 方案，價格可能落在每 Mbps 人民幣 300～2,000 元。由於專線頻寬非常昂貴，仍可使用這類軟體做流量分流，降低所需的專線頻寬。
...
當你在中國大陸部署雲端伺服器，卻卡在 npm、PyPI、GitHub、Docker 等海外相依套件下載時：
